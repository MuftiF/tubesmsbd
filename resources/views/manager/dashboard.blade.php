        ...
        <div class="bg-white rounded-xl shadow-md p-6 border border-gray-100">
            ...
            <div class="space-y-4">
                <a href="{{ route('manager.laporan') }}"
                    class="block bg-blue-600 hover:bg-blue-700 text-white rounded-lg py-4 px-4 text-center shadow-sm font-semibold transform hover:scale-[1.03] transition">
                    <div class="text-2xl mb-1">📊</div>
                    Laporan
                </a>

                <a href="{{ route('manager.log') }}"
                    class="block bg-green-600 hover:bg-green-700 text-white rounded-lg py-4 px-4 text-center shadow-sm font-semibold transform hover:scale-[1.03] transition">
                    <div class="text-2xl mb-1">📋</div>
                    Log Absensi
                </a>

                <a href="{{ route('manager.pegawai') }}"
                    class="block bg-purple-600 hover:bg-purple-700 text-white rounded-lg py-4 px-4 text-center shadow-sm font-semibold transform hover:scale-[1.03] transition">
                    <div class="text-2xl mb-1">👥</div>
                    Pegawai
                </a>
            </div>
        ...
        <div class="bg-white rounded-xl shadow-md p-6 border border-gray-100 lg:col-span-2">
            ...
            <div class="mt-4 grid grid-cols-3 gap-4 text-center">
                <div class="bg-green-50 p-3 rounded-lg border border-green-200">
                    ...
                    <p class="text-xl font-bold text-green-700" id="avgDaily">{{ number_format($avgProduksi ?? 0, 1) }} kg</p>
                </div>
                <div class="bg-blue-50 p-3 rounded-lg border border-blue-200">
                    <p class="text-sm text-gray-600">Total Bulan Ini</p>
                    <p class="text-xl font-bold text-blue-700" id="monthlyTotal">{{ number_format($totalProduksiBulanIni ?? 0, 1) }} kg</p>
                </div>
                <div class="bg-yellow-50 p-3 rounded-lg border border-yellow-200">
                    <p class="text-sm text-gray-600">Puncak Produksi</p>
                    <p class="text-xl font-bold text-yellow-700" id="peakProduction">{{ number_format($peakProduksi ?? 0, 1) }} kg</p>
                </div>
            </div>
        </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-10">
        ...
        <div class="bg-white rounded-xl shadow-md p-6 border border-gray-100">
            ...
            <div class="space-y-4">
                ...
                <div class="flex items-center justify-between bg-purple-50 px-4 py-3 rounded-lg border border-purple-200">
                    <div class="flex items-center">
                        <span class="text-purple-600 text-2xl mr-3">📈</span>
                        <p class="font-semibold text-gray-800">Trend Produksi</p>
                    </div>
                    <span class="bg-purple-100 text-purple-800 px-3 py-1 rounded-full text-sm font-bold" id="trendIndicator">
                        {{ $trend ?? 'Stabil' }}
                    </span>
                </div>
            </div>
        ...
        <div class="bg-white rounded-xl shadow-md p-6 border border-gray-100">
            ...
            <div class="space-y-3">
                ...
                <div class="flex items-center justify-between p-3 rounded-lg border {{ $index < 3 ? 'bg-green-50 border-green-200' : 'bg-gray-50 border-gray-200' }}">
                    <div class="flex items-center">
                        <div class="w-10 h-10 rounded-full {{ $index < 3 ? 'bg-yellow-100 text-yellow-800' : 'bg-gray-200 text-gray-600' }} flex items-center justify-center font-bold mr-3">
                            {{ $index + 1 }}
                        </div>
                        <div>
                    ...
                    <div class="text-right">
                        <p class="font-bold text-green-700">{{ number_format($performer->total_produksi ?? 0, 1) }} kg</p>
                        <p class="text-xs text-gray-500">{{ $performer->check_in_time ?? 'Belum absen' }}</p>
                    </div>
                </div>
                @empty
            ...
            <div class="mt-4 pt-4 border-t border-gray-200">
                <div class="flex justify-between text-sm text-gray-600">
                    ...
                    <span class="font-bold text-green-700">
                        {{ number_format($topPerformers->sum('total_produksi'), 1) }} kg
                    </span>
                </div>
            </div>
            @endif
